...
|---|---|---|---|
| t1 | `tool-originated-task-sessions-m2-t1` | [`t1-mode-affordance-render.md`](./t1-mode-affordance-render.md) | Landed |
...
- **D2 — The session-start integration is a Claude Code
  ...
  deterministic register subcommand with env-only slug
  carry.** *Resolved this session.* The hook is committed to
  the project-scoped `.claude/settings.json` (so every
  contributor working in this repo gets the deterministic-path
  registration automatically) with `type: "command"`,
  matcher `startup`, and a command that invokes
  `workstream-tracker register` **with no `--slug` flag** —
  the CLI's existing slug-resolution in
  [`runRegister`](../../../../cmd/workstream-tracker/register.go)
  already falls back from the `--slug` flag value to the
  `WST_SLUG` env var when the flag is absent, so the inherited
  `WST_SLUG` is picked up directly. The
  no-flag form is **load-bearing**, not stylistic: the
  alternative `register --slug $WST_SLUG` form fails open in
  shell expansion when `WST_SLUG` is unset — the empty
  expansion word-splits away and Go's `flag.Parse` errors with
  `flag needs an argument: -slug`, taking the incidental
  parse-error short-circuit rather than the intended "no slug
  supplied" branch this contract leans on for the unset-case
  preserves-clause below. (The agent rule's deterministic-path
  example shows the `--slug <slug>` form for the manual / CLI
  case where a literal value is substituted; the hook context
  — env-var carry, no agent cognition — uses env-only carry of
  the same identity mechanism. See t3 for the small additive
  clarification this may add to the agent rule, per t3's
  contract.) The command is invoked via the agent rule's full
  module-path form
  ([`session-registration.md` "The deterministic path"](../../../../docs/agents/local/session-registration.md)).
...
- **D4 — Spawn-mechanism shape: server-side fire-and-forget
  ...
  *human-initiated* (the click is the contributor's explicit
  ...
  `runServer` (the chi-router host the new POST handler mounts
  onto; the **current** binding `addr := ":" + port` is
  all-interfaces, not loopback — t2's contract constrains it
  to loopback-only at `/spawn`-mount per the Cross-Task
  Invariant above).
...
- **New `/spawn` endpoint expands the tool's write surface
  ...
  **defense-in-depth-validated** against the slug grammar and
  ...
  forbids); the exec is bounded to the verbatim `claude --bg …`
  argument shape (no shell interpolation of caller data into
  a shell command line — use `os/exec.Command` with a fixed
  `argv`); the fire-and-forget posture means the supervisor
  (`claude daemon status`) — not the workstream-tracker —
  owns the agent's process lifetime, so the new surface does
  not accumulate persistent state. The agent-adapter-seam
