...
| `workstream-tracker-1-0-m1-t2`    | Automatic agent registration         | [Proposed](m1-t2-auto-registration.md) |
| `workstream-tracker-1-0-m1-t3`    | Descriptive tree labels              | [Landed](m1-t3-descriptive-labels.md) |
| `workstream-tracker-1-0-m1-t4`    | Expanded per-node display            | [In progress](m1-t4-expanded-per-node-display.md) |
...
(Status `In progress`; orchestration contracts
decision-complete and promotion-gate self-review run — it
flipped `Proposed → In progress` when P1's implementing PR
merged and reaches `Landed` with P2's; P2's phase plan drafts
just-in-time after P1's implementing PR merges, which is a
future just-in-time doc, not a hold on the task plan); the P1
phase plan
[`m1-t4-p1-inline-detail-render.md`](m1-t4-p1-inline-detail-render.md)
is Status `Landed` (P1's implementing PR). t4
...
- **End result.** Agent sessions register without manual API
  ...
  milestone reconciliation; rationale in
...
- **Slug derivation for auto-registration (t2): RESOLVED at t2
  ...
  [`scoping/m1-t2-auto-registration.md`](scoping/m1-t2-auto-registration.md)
  Decision 2 and the t2 plan's C2.
...
- [`../../../AGENTS.md`](../../../AGENTS.md) and
  ...
  read-only; corrected per the t2 scoping doc's Decision 6 and
  the t2 plan.
