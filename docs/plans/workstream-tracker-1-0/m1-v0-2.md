...
(Status `In progress`; it flipped `Proposed → In progress`
when P1's implementing PR merged and reaches `Landed` with
...
Status `Proposed` (drafted just-in-time after P1 merged; the
scoping-D5 novel-mechanism spike run, decisions resolved,
promotion-gate self-review complete). t4
