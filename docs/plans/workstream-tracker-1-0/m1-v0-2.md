...
| `workstream-tracker-1-0-m1-t4`    | Expanded per-node display            | [Proposed](m1-t4-expanded-per-node-display.md) |
...
(Status `Proposed`; orchestration contracts decision-complete
and promotion-gate self-review run — P2's phase plan drafts
